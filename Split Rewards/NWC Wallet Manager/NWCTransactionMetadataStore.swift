//
//  NWCTransactionMetadataStore.swift
//  Split Rewards
//
...
struct NWCTransactionMetadata: Codable, Equatable {
    ...
    func merging(metadata: NWCTransactionMetadata) -> NWCTransactionMetadata {
    ...
    func withReportable(_ isReportable: Bool) -> NWCTransactionMetadata {
        NWCTransactionMetadata(
            walletId: walletId,
    ...
    func withUserLog(_ userLog: String?) -> NWCTransactionMetadata {
        NWCTransactionMetadata(
            walletId: walletId,
            transactionId: transactionId,
            transactionType: transactionType,
    ...
    func withDestinationMetadata(destinationPubkey: String?, paymentHash: String?) -> NWCTransactionMetadata {
        NWCTransactionMetadata(
            walletId: walletId,
            transactionId: transactionId,
            transactionType: transactionType,
...
private struct SecureNWCTransactionMetadataStorage {
    static let shared = SecureNWCTransactionMetadataStorage()

    private let storageKeyKeychainKey = "split.nwc.transactions.storageKey.v1"
    private let payloadHeader = Data("SNWCM1".utf8)
    ...
    enum SecureStorageError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .invalidStoredKey:
                return "Stored NWC transaction metadata key is invalid."
            case .missingStoredKey:
                return "Stored NWC transaction metadata key is missing."
            case .invalidEncryptedPayload:
                return "Encrypted NWC transaction metadata payload is invalid."
            case .failedToEncrypt:
                return "Could not encrypt local NWC transaction metadata."
            case .failedToDecrypt:
                return "Could not decrypt local NWC transaction metadata."
            }
        }
    }
    ...
    func clearStoredKey() {
        NWCTransactionMetadataKeychain.delete(forKey: storageKeyKeychainKey)
    }

    private func storageKey(createIfMissing: Bool) throws -> SymmetricKey {
        if let stored = NWCTransactionMetadataKeychain.read(forKey: storageKeyKeychainKey) {
        ...
        NWCTransactionMetadataKeychain.save(keyData.base64EncodedString(), forKey: storageKeyKeychainKey)
        return newKey
    }
}

actor NWCTransactionMetadataStore {
    static let shared = NWCTransactionMetadataStore()

    private var metadataByCompositeKey: [String: NWCTransactionMetadata]?
    private let encoder = JSONEncoder()
    private let decoder = JSONDecoder()

    private init() {}

    func metadata(walletId: String, transactionIds: [String]) -> [String: NWCTransactionMetadata] {
        let allMetadata = loadMetadata()
        return transactionIds.reduce(into: [String: NWCTransactionMetadata]()) { partialResult, transactionId in
            if let metadata = allMetadata[compositeKey(walletId: walletId, transactionId: transactionId)] {
                partialResult[transactionId] = metadata
            }
        }
    }

    func reportableStates(walletId: String, transactionIds: [String]) -> [String: Bool] {
        let allMetadata = loadMetadata()
    ...
    func userLogs(walletId: String, transactionIds: [String]) -> [String: String] {
        let allMetadata = loadMetadata()
        return transactionIds.reduce(into: [String: String]()) { partialResult, transactionId in
            guard let metadata = allMetadata[compositeKey(walletId: walletId, transactionId: transactionId)],
    ...
    func setReportable(
        walletId: String,
        transactionId: String,
        transactionType: NWCTransactionMetadata.TransactionType,
        isReportable: Bool
    ) {
        var allMetadata = loadMetadata()
        let key = compositeKey(walletId: walletId, transactionId: transactionId)

        if let existing = allMetadata[key] {
            allMetadata[key] = existing.withReportable(isReportable)
        } else {
            allMetadata[key] = NWCTransactionMetadata(
                walletId: walletId,
                transactionId: transactionId,
                transactionType: transactionType,
                isReportable: isReportable
    ...
    func setUserLog(
        ...
        var allMetadata = loadMetadata()
        let key = compositeKey(walletId: walletId, transactionId: transactionId)
        let normalizedUserLog = NWCTransactionMetadata.normalizedUserLog(userLog)
    ...
    func setDestinationMetadata(
        ...
        let normalizedDestinationPubkey = NWCTransactionMetadata.normalizedUserLog(destinationPubkey)
        let normalizedPaymentHash = NWCTransactionMetadata.normalizedUserLog(paymentHash)
    ...
    func clearAll() {
        ...
        do {
            ...
            SecureNWCTransactionMetadataStorage.shared.clearStoredKey()
        } catch {
            print("NWCTransactionMetadataStore clear failed: \(error)")
        }
    }

    private func compositeKey(walletId: String, transactionId: String) -> String {
        "\(walletId)|\(transactionId)"
    }

    private func containsUsdSnapshot(walletId: String, transactionId: String) -> Bool {
        let key = compositeKey(walletId: walletId, transactionId: transactionId)
        return loadMetadata()[key]?.hasUsdSnapshot == true
    }

    private func upsert(_ metadata: NWCTransactionMetadata) {
        var allMetadata = loadMetadata()
        let key = compositeKey(walletId: metadata.walletId, transactionId: metadata.transactionId)
        if let existing = allMetadata[key] {
    ...
    private func persistUsdSnapshotIfNeeded(
        ...
        guard !containsUsdSnapshot(walletId: walletId, transactionId: row.id) else { return }
        ...
        do {
            ...
            print("NWCTransactionMetadataStore failed to fetch BTC/USD snapshot for \(row.id): \(error.localizedDescription)")
            return
        }
        ...
        let transactionType: NWCTransactionMetadata.TransactionType = row.direction == "sent" ? .sent : .received
    ...
    private func loadMetadata() -> [String: NWCTransactionMetadata] {
        ...
        do {
            ...
            let isEncrypted = SecureNWCTransactionMetadataStorage.shared.isEncryptedPayload(data)
            let plaintextData = isEncrypted
                ? try SecureNWCTransactionMetadataStorage.shared.decrypt(data)
                : data
            let metadata = try decoder.decode([String: NWCTransactionMetadata].self, from: plaintextData)
            ...
            print("NWCTransactionMetadataStore load failed: \(error)")
            metadataByCompositeKey = [:]
            return [:]
        }
    }

    private func persistMetadata(_ metadata: [String: NWCTransactionMetadata]) {
        do {
            let fileURL = try storageFileURL()
            let data = try encoder.encode(metadata)
            let encryptedData = try SecureNWCTransactionMetadataStorage.shared.encrypt(data)
            try encryptedData.write(to: fileURL, options: [.atomic])
            try protectMetadataFile(at: fileURL)
        } catch {
            print("NWCTransactionMetadataStore persist failed: \(error)")
        }
    }
    ...
    private func storageFileURL() throws -> URL {
        ...
        let directory = appSupport.appendingPathComponent("nwc-transaction-metadata", isDirectory: true)
...
private enum NWCTransactionMetadataKeychain {
    private static let service = "com.split.rewards.nwc"
